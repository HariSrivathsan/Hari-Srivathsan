# Hi there, I'm Hari Srivathsan 👋

## About Me

I'm a **B.E. in Electronics and Communication Engineering (ECE)** graduate, and I'm currently pursuing an **Embedded Systems course** to further enhance my skills in the world of embedded technology.

I have a deep passion for **electronics**, **embedded systems**, and **IoT**. I enjoy working on both hardware and software to bring innovative ideas to life.

- 🔭 Currently learning **Embedded Systems**, **Microcontroller Programming**, **IoT** (Internet of Things), and **Real-Time Operating Systems (RTOS)**.
- 🌱 Actively working on projects involving **Arduino**, **Raspberry Pi**, **Embedded C**, and **IoT applications**.
- 👯 Looking for opportunities to collaborate on projects related to **Embedded Systems**, **IoT**, **Automation**, and **Sensor Integration**.
- 💬 Feel free to ask me about **C programming**, **Embedded Systems Development**, **Arduino Projects**, and more!
- 📫 You can reach me at [harisrivathsan.pers@gmail.com](mailto:your-email@example.com).

## Skills & Tools

- **Languages**: C, C++, Embedded C
- **Embedded Platforms**: Proteus, Flashmagic, Keil
- **Tools/Technologies**: Embedded C, IoT, RTOS
- **Microcontrollers**: ARM7
- **Operating Systems**: Linux (Ubuntu), Windows

## Projects

Here are some of the cool projects I’ve worked on:

### 1. [Embedded Home Automation System](https://github.com/your-github-repo)
A home automation system built using **Arduino** and **IoT** for smart home control.


## Contact Me

- GitHub: https://github.com/HariSrivathsan
- LinkedIn: linkedin.com/in/hari-srivathsan
- Email: harisrivathsan.pers@gmail.com

## Education

- **B.E. in Electronics and Communication Engineering (ECE)** - [Easwari Engineering College]
- **Embedded Systems Course** (Ongoing) - [Vector India ]

---

Feel free to connect with me! I'm always excited to collaborate on new projects and learn from others in the tech community. 🚀
